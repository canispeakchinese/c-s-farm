...
MainView::MainView(QGraphicsView *parent) : QGraphicsView(parent), tcpClient(new QTcpSocket), login(new Login())
{
    ...
    connect(login, SIGNAL(sendLoginRequest(QString,QString,bool)), this, SLOT(sendLoginRequest(QString,QString,bool)));
}

void MainView::getLoginResult(QDataStream &in, bool isSign)
{
    if(!isSign)
    {
        bool ok;
    ...
    else
    {
        ...
        if(type == 1)
        {
            QMessageBox::warning(this, "注册失败", "帐号已存在");
            return;
        }
        else
        {
            int id;
            in >> id;
            person = new PersonDefin(username, QString(":face/image/face/1-1.GIF"), 1, 0, 30);
            login->close();
        }
    }
    initMainView();
...
void MainView::sendLoginRequest(QString _username, QString _password, bool isSignin)
{
    username = _username;
    password = _password;
    QByteArray outBlock;
...
void MainView::readyRead()
{
    ...
    if(inBlock.size() >= totalBytes - 12)
    {
        ...
        switch (messageType) {
        case 1:
            getLoginResult(in, false);
            break;
        case 2:
            getLoginResult(in, true);
            break;
        case 3:
            getUpdateResult(in);
            break;
