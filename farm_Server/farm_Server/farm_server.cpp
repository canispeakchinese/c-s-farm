...
void FarmServer::messageCallback(const TcpConnectionPtr &conn, Buffer *buf, Timestamp time) {
    ...
    while(true) {
        ...
        if(messageType > 2) {
            LOG_INFO << "User " << clientInfor.id << " send a request, and now confirm password";
            int id;
            QString password;
            in >> id >> password;
            if(id != clientInfor.id || password != clientInfor.password) {
                LOG_INFO << "User " << clientInfor.id << "'s information is error, force close this connection";
                conn->forceClose();
                {
                    MutexLockGuard mutexLockGuard(mutex);
                    clientMap.erase(conn);
            ...
            LOG_INFO << "User " << clientInfor.id << "'s information is right";
        }
